...
If SED->ED_ZZCTB$"C,F"
	If MsgYesNo("O uso desta natureza geralmente acompanha uma NF, a digita็ใo da mesma deve ser pelo m๓dulo " + ;
		"de Compras ou Faturamento para o correto processamento do sistema. Continuar?","ATENCAO")
	Else
		RestArea(_xAreaSED)
		Return(.f.)
	EndIf
...
If Empty(_cConta) // Conta nใo foi informa pelo usuแrio.
	...
	If Found() // Encontrou a natureza, obt้m as contas e valida.
		...
		If _cRegraCC $ "ACO"  .And. !Empty(SED->ED_ZZCTAD) // Administra็ใo / Comercial / Outras=Financeiras
