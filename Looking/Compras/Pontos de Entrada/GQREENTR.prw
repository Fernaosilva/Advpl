...
If SF1->F1_FORMUL == "S"
   ...
   @ 095,080 To 330,465 Dialog JANELANF Title OemToAnsi("Complemetacao de Dados da Nota Fiscal de Entrada")
...
//   @ 080,090 Say OemToAnsi("Numero")
   
   

   @ 020,040 Get _cMarca   Picture "@!"             SIZE 20,10
   @ 020,120 Get _nVolume  Picture "99999"          SIZE 20,10
   @ 040,040 Get _cEspecie Picture "@!"             SIZE 40,10
   @ 040,120 Get _cTransp  Picture "@!" F3 "SA4"    SIZE 20,10
   @ 060,040 Get _nPBruto  Picture "@E 999,999.999" SIZE 40,10 
   @ 060,120 Get _nPLiqui  Picture "@E 999,999.999" SIZE 40,10
   @ 080,040 Get _cPlaca   Picture "@!"             SIZE 40,10 
//   @ 080,120 Get _cnumero  Picture "@!"             SIZE 40,10 

   @ 100,120 BmpButton Type 1 Action Close(JANELANF)
   ...
   RecLock("SF1",.F.)
   SF1->F1_ZZESPEC  := _cEspecie
   SF1->F1_ZZPBRUT  := _nPBruto   
   SF1->F1_PESOL    := _nPLiqui
   SF1->F1_ZZMARCA  := _cMarca
   SF1->F1_ZZVOLUM  := _nVolume
   SF1->F1_ZZTRANS  := _cTransp
   SF1->F1_ZZPLACA  := _cPlaca
//   SF1->F1_SCNUMER  := _cNumero
   MsUnlock()
