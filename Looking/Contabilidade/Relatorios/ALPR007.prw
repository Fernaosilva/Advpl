...
±±³Fun‡„o³ ALPR007	³ Autor ³ Natanael Delatorre      Data ³ 04.01.08 ³±±
±±³Programa  ³ ALPR003  ³ Alterado por Donizete/Marcondes        19/03/08 ³±±
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
wnrel := "ALPR007"            //Nome Default do relatorio em Disco
...
nSaldoAtu:=Round(xMoeda(aAlpSdo[1],nMoedaBco,mv_par06,aAlpSdo[2],nMoeda+1),nMoeda)
nSaldoIni:=nSaldoAtu
...
If Empty(xFilial( "SA6")) .and. !Empty(xFilial("SE5"))
	...
	cChave	:= "DTOS(E5_DATA)+E5_BANCO+E5_AGENCIA+E5_CONTA"
	lAllFil:= .T.
Else
...
#IFNDEF TOP
	If  lAllFil
		...
		cCondWhile := "!Eof() .And. E5_DATA <= mv_par05"
	Else
		//cCondWhile := "!Eof() .And. E5_FILIAL == xFilial('SE5') .And. E5_DTDISPO <= mv_par05"
		cCondWhile := "!Eof() .And. E5_FILIAL == xFilial('SE5') .And. E5_DATA <= mv_par05"
	EndIf
#ELSE
	If TcSrvType() != "AS/400"
		DbSelectArea("SE5")
		...
		If	lAllFil
			...
			cChaveSQL  := "DTOS(E5_DATA)+E5_BANCO+E5_AGENCIA+E5_CONTA+E5_NUMCHEQ+R_E_C_N_O_"
		Else
			//cChaveSQL  := "E5_FILIAL+DTOS(E5_DTDISPO)+E5_BANCO+E5_AGENCIA+E5_CONTA+E5_NUMCHEQ+R_E_C_N_O_"
			cChaveSQL  := "E5_FILIAL+DTOS(E5_DATA)+E5_BANCO+E5_AGENCIA+E5_CONTA+E5_NUMCHEQ+R_E_C_N_O_"
		EndIf
		...
		cQuery += " AND E5_DATA >=  '"     + DTOS(mv_par04) + "'"
		//If lSpbInuse
...
While &(cCondWhile)
   ...
   //ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	...
	ElseIf cPaisLoc=="BRA"  .Or. cPaisLoc<>"BRA" .And. (Empty(E5_MOEDA) .Or. (!Empty(E5_MOEDA).And.Val(E5_MOEDA)==1) )
	...
	nTxMoeda := RecMoeda(E5_DATA,mv_par06)

	//nValor := Round(xMoeda(E5_VALOR,Iif((cPaisLoc<>"BRA".And.(E5_TIPODOC $ MVRECANT+"|ES") .and. Empty(E5_MOEDA)),1,nMoedaBco),mv_par06,E5_DTDISPO,nMoeda+1,,nTxMoedBc),nMoeda)
	nValor := Round(xMoeda(E5_VALOR,Iif((cPaisLoc<>"BRA".And.(E5_TIPODOC $ MVRECANT+"|ES") .and. Empty(E5_MOEDA)),1,nMoedaBco),mv_par06,E5_DATA,nMoeda+1,,nTxMoedBc),nMoeda)
...
Static Function CompSdAnt()
Local cQry		:= ''
Local cAlias	:= GetNextAlias()
Local aRet		:= {}
Local aArea		:= GetArea()

cQry	:= 'SELECT SE5.E5_BANCO, SE5.E5_AGENCIA, SE5.E5_CONTA, '

cQry	+= "SUM( CASE WHEN SE5.E5_RECPAG = 'R' THEN SE5.E5_VALOR "
cQry	+= "          WHEN SE5.E5_RECPAG = 'P' THEN SE5.E5_VALOR * -1 "
cQry	+= "          ELSE 0 END) E5_VALOR, "
cQry	+= "MAX( SE5.E5_DATA) ULTDATA "
cQry	+= "FROM " + RetSqlName("SE5") + " SE5 "
cQry	+= "WHERE SE5.E5_FILIAL = '"	+ xFilial("SE5") + "' "
cQry	+= "  AND SE5.E5_BANCO = '"		+ MV_PAR01 + "' "
cQry	+= "  AND SE5.E5_AGENCIA = '"	+ MV_PAR02 + "' "
cQry	+= "  AND SE5.E5_CONTA = '"		+ MV_PAR03 + "' "
cQry	+= "  AND SE5.E5_DATA < '"		+ Dtos( MV_PAR04) + "' "
cQry	+= "  AND SE5.E5_SITUACA <> 'C' "
cQry	+= "  AND SE5.E5_TIPODOC NOT IN ('DC','JR','MT','CM','D2','J2','M2','C2','V2','CP','TL','BA') "
cQry	+= "  AND E5_NUMCHEQ NOT LIKE '%*' "
cQry	+= "  AND SE5.D_E_L_E_T_ = ' ' "

cQry	+= "GROUP BY SE5.E5_BANCO, SE5.E5_AGENCIA, SE5.E5_CONTA"
...
aRet	:= { ( cAlias)->E5_VALOR, Stod( ( cAlias)->ULTDATA)}
